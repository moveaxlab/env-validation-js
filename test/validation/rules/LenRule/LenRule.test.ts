...
import { LenRule } from '../../../../src/validation/rules/index';
import {
  LowLevelSpec,
  HighLevelSpec,
  TypeNames,
...
describe(`Test ${LenRule.name} rule`, function() {
  ...
  const rule = new LenRule(type, testData, '');
  ...
  describe(`Checking data`, function() {
    ...
    it(`check data ${data1} is of exact length ${testData}`, function() {
      try {
        rule.apply(data1);
      } catch (e) {
        assert(false);
    ...
    const data2 = 'worlds';
    it(`check data ${data2} is not of exact length ${testData}`, function() {
      try {
        rule.apply(data2);
      } catch (e) {
        assert(true);
