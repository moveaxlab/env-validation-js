...
import { RegexRule } from '../../../../src/validation/rules/index';
import {
  LowLevelSpec,
  HighLevelSpec,
  TypeNames,
...
describe(`Test ${RegexRule.name} rule`, function() {
  ...
  const rule = new RegexRule(type, testData, '');
  ...
  describe(`Checking data`, function() {
    const data1 = 'AAAABBBBB';

    it(`check data ${data1} is valid in pattern ${testData}`, function() {
      try {
        rule.apply(data1);
      } catch (e) {
        assert(false);
    ...
    const data2 = 'AAACBB';
    it(`check data ${data2} is not valid in pattern ${testData}`, function() {
      try {
        rule.apply(data2);
      } catch (e) {
        assert(true);
