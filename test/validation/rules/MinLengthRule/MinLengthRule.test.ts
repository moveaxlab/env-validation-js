...
import { MinLengthRule } from '../../../../src/validation/rules/index';
import {
  LowLevelSpec,
  HighLevelSpec,
  TypeNames,
...
describe(`Test ${MinLengthRule.name} rule`, function() {
  ...
  const rule = new MinLengthRule(type, testData, '');
  ...
  describe(`Checking data`, function() {
    ...
    it(`check data ${data1} is of length min ${testData}`, function() {
      try {
        rule.apply(data1);
      } catch (e) {
        assert(false);
    ...
    const data2 = 'word';
    it(`check data ${data2} is not of length min ${testData}`, function() {
      try {
        rule.apply(data2);
      } catch (e) {
        assert(true);
