...
import { MaxSizeRule } from '../../../../src/validation/rules/index';
import {
  LowLevelSpec,
  HighLevelSpec,
  TypeNames,
...
describe(`Test ${MaxSizeRule.name} rule`, function() {
  ...
  const rule = new MaxSizeRule(type, testData, '');
  ...
  describe(`Checking data`, function() {
    ...
    it(`check data ${data1.substr(
      0,
      100
    )}... is less or equal than max size ${testData}`, function() {
      try {
        rule.apply(data1);
      } catch (e) {
        assert(false);
    ...
    it(`check data ${data2.substr(
      0,
      100
    )}... is not less or equal than max size ${testData}`, function() {
      try {
        rule.apply(data2);
      } catch (e) {
        assert(true);
