...
import { MinSizeRule } from '../../../../src/validation/rules/index';
import {
  LowLevelSpec,
  HighLevelSpec,
  TypeNames,
...
describe(`Test ${MinSizeRule.name} rule`, function() {
  ...
  const type = new Base64EncodedFileType(llschema1);

  const rule = new MinSizeRule(type, testData, '');
  ...
  describe(`Checking data`, function() {
    ...
    const data2 =
      'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAJAAAACQCAYAAADnRuK4AAATdElEQVR4nO2de3BU5d3Hv+fsZjcwARIujkoLIVJJG7wg3pipSOcVR9RACq4MFzWE6OsNi6ISJdrTl4oolVqcighUvMKk046MbUClxguiM2+Mbii1EhQBfYXsnk0gyd7P833/OLtLIAEhezZ7Yp/';
    it(`check data ${data2.substr(
      0,
      100
    )}... is not greater or equal than min size ${testData}`, function() {
      try {
        rule.apply(data2);
      } catch (e) {
        assert(true);
